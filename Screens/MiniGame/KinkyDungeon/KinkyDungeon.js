...
function KinkyDungeonRun() {
	...
	if (KinkyDungeonState == "Credits") {
		...
		if (KinkyDungeonAction > KinkyDungeonClock) {
			if (CommonTime() > KinkyDungeonSleepTime) {
				KinkyDungeonAdvanceTime(0.5);
				KinkyDungeonSleepTime = CommonTime() + Math.max(10, 150 - 50 * (KinkyDungeonAction - KinkyDungeonClock));
			}
...
function KinkyDungeonHandleClick() {
	for (let I = 0; I < KinkyDungeonButtons.length; I++) {
		let button = KinkyDungeonButtons[I];
		if (MouseIn(button.x, button.y, button.width, button.height)) {
			if (button.tag.startsWith("To")) {
				KinkyDungeonDrawState = button.tag.slice(2);
				return true;
			}
			if (button.tag.startsWith(KinkyDungeonDrawState)) {
				if (typeof (window[`KinkyDungeonButton${KinkyDungeonDrawState}`]) === "function")
					return window[`KinkyDungeonButton${KinkyDungeonDrawState}`]({tag:button.tag.slice(KinkyDungeonDrawState.length), key:button.key});
				console.log("unhandled button press:"+button.tag);
				return false;
			}
		}
	}
	if (MouseIn(1885, 25, 90, 90) && (KinkyDungeonDrawState == "Game" || KinkyDungeonState != "Game") && ServerURL != "foobar") {
	...
	if (KinkyDungeonState == "Credits") {
		...
		if (MouseIn(875, 750, 350, 64)) {
			KinkyDungeonSendMessage(10, TextGet("KinkyDungeonSavedGame"), "white", 1);
...
function KinkyDungeonSaveGame(ToString) {
	...
	save.stats = {
		...
		willpower: KinkyDungeonStatWillpower,
		willpowermax: KinkyDungeonStatWillpowerMax,
		stamina: KinkyDungeonStatStamina,
		staminamax: KinkyDungeonStatStaminaMax,
		arousal: KinkyDungeonStatArousal,
		arousalmax: KinkyDungeonStatArousalMax,
	...
	if (str) {
		...
		if (saveData
			...
			if (saveData.stats) {
				...
				if (saveData.stats.manamax != undefined) KinkyDungeonStatManaMax = saveData.stats.manamax;
				if (saveData.stats.stamina != undefined) KinkyDungeonStatStamina = saveData.stats.stamina;
				if (saveData.stats.staminamax != undefined) KinkyDungeonStatStaminaMax = saveData.stats.staminamax;
				if (saveData.stats.arousal != undefined) KinkyDungeonStatArousal = saveData.stats.arousal;
				if (saveData.stats.arousalmax != undefined) KinkyDungeonStatArousalMax = saveData.stats.arousalmax;
				if (saveData.stats.willpower != undefined) KinkyDungeonStatWillpower = saveData.stats.willpower;
				if (saveData.stats.willpowermax != undefined) KinkyDungeonStatWillpowerMax = saveData.stats.willpowermax;

...
function KinkyDungeonHandleInventory() {
	...
	if (KinkyDungeonDrawInventorySelected(filteredInventory)) {
		if (KinkyDungeonCurrentFilter == "Consumables" && MouseIn(canvasOffsetX + 640*KinkyDungeonBookScale + 25, canvasOffsetY + 483*KinkyDungeonBookScale, 350, 60)) {
			...
			if (!KinkyDungeonAttemptConsumable(item.name, 1)) {
			...
			if (weapon && weapon != "Knife") {
				...
				if (MouseIn(canvasOffsetX + 640*KinkyDungeonBookScale + 25, canvasOffsetY + 483*KinkyDungeonBookScale, 350, 60) && !equipped) {
					...
					KinkyDungeonSendMessage(7, TextGet("KinkyDungeonEquipWeapon").replace("WEAPONNAME", TextGet("KinkyDungeonInventoryItem" + weapon)), "white", 5);
					...
					KinkyDungeonSendMessage(7, TextGet("KinkyDungeonUnEquipWeapon").replace("WEAPONNAME", TextGet("KinkyDungeonInventoryItem" + weapon)), "white", 5);
				}
			}
			...
			let looserestraint = KinkyDungeonFilterInventory(KinkyDungeonCurrentFilter)[KinkyDungeonCurrentPageInventory];
			if (!looserestraint || !looserestraint.name) return true;

			if (!KinkyDungeonPlayer.CanInteract()) {
				KinkyDungeonSendMessage(7, TextGet("KinkyDungeonCantUseRestraints"), "red", 2);
			} else {
				// bind this back
				KinkyDungeonDoAction("Interact");				

				KinkyDungeonAddRestraintIfWeaker(looserestraint.item.looserestraint, 0, true, false);
				KinkyDungeonLock(KinkyDungeonGetRestraintItem(looserestraint.item.looserestraint.Group),"");

				for (let II = 0; II < KinkyDungeonInventory.length; II++) {
					if (KinkyDungeonInventory[II].looserestraint && KinkyDungeonInventory[II].looserestraint.name == looserestraint.name) {
						KinkyDungeonInventory.splice(II, 1);
						break;
					}
				}
				KinkyDungeonSendMessage(7, TextGet("KinkyDungeonUseRestraints").replace("RESTRAINT", TextGet("Restraint"+looserestraint.item.looserestraint.name)), "red", 2);
			}
			KinkyDungeonDrawState = "Game";
			return true;
		}
...
function KinkyDungeonDrawInventory() {
	...
	if (KinkyDungeonDrawInventorySelected(filteredInventory)) {
		...
		if (KinkyDungeonCurrentFilter == "Restraints")
			DrawButton(canvasOffsetX + 640*KinkyDungeonBookScale + 25, canvasOffsetY + 483*KinkyDungeonBookScale, 350, 60, TextGet("KinkyDungeonRestraints"), "White", "", "");
	}

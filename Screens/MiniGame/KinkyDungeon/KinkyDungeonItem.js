...
function KinkyDungeonDropItem(Item) {
	...
	if (foundslot) {
		...
		KinkyDungeonGroundItems.push(dropped);
		KinkyDungeonSendMessage(7, TextGet("KinkyDungeonDrop" + Item.name), "red", 2);
...
function KinkyDungeonItemEvent(Item) {
	...
	KinkyDungeonSendMessage(priority, TextGet("ItemPickup" + Item.name).replace("XXX", Item.amount), color, 2);
}
